...
void test_cropenat_struct(void)
{
	...
	int base_fd = open(".", O_RDONLY);
	if (base_fd < 0) {
		ksft_print_msg("failed to open CWD\n");
		return;
	}
	...
	for (int i = 0; i < ARRAY_LEN(tests); i++) {
		...
		for (int j = 0; j < ARRAY_LEN(misalignments); j++) {
			...
			if (!cropenat_supported) {
				ksft_print_msg("cropenat(2) unsupported\n");
			...
			fd = raw_cropenat(base_fd, ".", how_copy, test->size);
			if (test->err >= 0)
				failed = (fd < 0);
			else
				failed = (fd != test->err);
			...
			if (failed) {
				...
				ksft_print_msg("cropenat unexpectedly returned ");
				if (fdpath)
					ksft_print_msg("%d['%s']\n", fd, fdpath);
				else
					ksft_print_msg("%d (%s)\n", fd, strerror(-fd));
...
skip:
			if (test->err >= 0)
				resultfn("cropenat with %s argument [misalign=%d] succeeds\n",
					 test->name, misalign);
			else
				resultfn("cropenat with %s argument [misalign=%d] fails with %d (%s)\n",
...
void test_cropenat_flags(void)
{
	...
	for (int i = 0; i < ARRAY_LEN(tests); i++) {
		...
		path = (test->how.flags & O_CREAT) ? "/tmp/ksft.cropenat_tmpfile" : ".";
		unlink(path);

		fd = sys_cropenat(base_fd, path, &test->how);
		if (test->err >= 0)
			failed = (fd < 0);
		else
			failed = (fd != test->err);
...
skip:
		if (test->err >= 0)
			resultfn("cropenat with %s succeeds\n", test->name);
		else
			resultfn("cropenat with %s fails with %d (%s)\n",
